...
public class WebScreenshotUtils {
    ...
    public static String saveWebPageScreenshot(String webUrl) {
        ...
        try {
            ...
            String compressedRelativePath = File.separator + RandomUtil.randomNumbers(5) + COMPRESSION_SUFFIX;
            String compressedImagePath = rootPath + compressedRelativePath;
            compressImage(imageSavePath, compressedImagePath);
            log.info("压缩图片保存成功: {}", compressedImagePath);
            // 删除原始图片，只保留压缩图片
            FileUtil.del(imageSavePath);
            return relativePath+compressedRelativePath;
        } catch (Exception e) {
